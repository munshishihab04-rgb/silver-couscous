...
function StatusBanner({ status }) {
  ...
  const rows = [
    ...
    { k: "Email (Brevo)", v: status.email_delivery_mode === "dry-run" ? "dry-run" : (status.email_configured ? "live" : "MANCA API key"), ok: status.email_delivery_mode === "dry-run" || status.email_configured },
    ...
    { k: "Chiavi reali", v: `${status.inventory_counts?.available || 0}`, ok: (status.inventory_counts?.available || 0) > 0 },
    { k: "Cifratura inventario", v: status.inventory_encryption_configured ? "configurata" : "MANCA", ok: status.inventory_encryption_configured },
