#!/usr/bin/env python3
"""Replace pilot images with deterministic LicenzPol-owned artwork."""
...
PILOT = json.loads((BACKEND / "data" / "pilot_catalog.json").read_text(encoding="utf-8"))
ASSETS = ROOT / "frontend" / "public" / "products"
PUBLIC_RECEIPT = BACKEND / "data" / "pilot_image_generation_receipt.json"
...
def main() -> None:
    ...
    for item in PILOT["items"]:
        ...
        receipt = render_product_image(item["name"], item["brand"], output)
        records.append({
            "slug": item["slug"],
            "asset_path": f"/products/{item['slug']}.webp",
            **receipt,
        })
    payload = {
        "version": 1,
    ...
    PRIVATE_RECEIPT.write_text(json.dumps({
        **payload,
        "reviewed_by": "automation:hermes-agent",
        "review_note": "Generated locally for DIGITALSOFT/LicenzPol at the user's explicit request; no external image inputs were used.",
