...
async def send_email(
    *,
    ...
    if resp.status_code >= 400:
        ...
        log.error("Brevo API %s: %s", resp.status_code, body)
        raise BrevoError(f"Brevo HTTP {resp.status_code}: {body}")
    data = resp.json()
    msg_id = data.get("messageId") or data.get("messageIds", [None])[0]
    if not msg_id:
        raise BrevoError(f"Brevo response missing messageId: {data}")
    return str(msg_id)
...
def order_confirmation_html(*, customer_name: str, order_ref: str, total_eur: float, items: list, delivery_note: str = "") -> str:
    rows = "".join(
        f'<tr><td style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.05);">{i.get("product_name","")}<br><span style="color:#71717a;font-size:12px;">{i.get("variant_label","")} × {i.get("quantity",1)}</span></td>'
        f'<td style="padding:8px 0;text-align:right;border-bottom:1px solid rgba(255,255,255,0.05);">{(i.get("unit_price_eur",0)*i.get("quantity",1)):.2f} €</td></tr>'
        for i in items
    )
    inner = f"""
      <h1 style="font-size:22px;color:#fff;margin:0 0 12px;">Ciao {customer_name},</h1>
      <p>abbiamo ricevuto il tuo ordine <strong>#{order_ref}</strong> e stiamo lavorando alla consegna.</p>
      <table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:14px;">
        {rows}
        <tr><td style="padding:12px 0 0;font-weight:700;color:#fff;">Totale</td>
            <td style="padding:12px 0 0;text-align:right;font-weight:700;color:#fff;">{total_eur:.2f} €</td></tr>
      </table>
      <p style="margin:16px 0 0;">{delivery_note}</p>
    """
...
def license_delivery_html(*, customer_name: str, order_ref: str, product_name: str, license_key: str, activation_link: Optional[str] = None) -> str:
    activation = f'<p style="margin:12px 0 0;"><a href="{activation_link}" style="color:#a78bfa;">Segui la guida di attivazione</a></p>' if activation_link else ""
    inner = f"""
      <h1 style="font-size:22px;color:#fff;margin:0 0 12px;">Ciao {customer_name}, la tua licenza è pronta.</h1>
      <p>Ecco la chiave di attivazione per <strong>{product_name}</strong> — ordine <strong>#{order_ref}</strong>.</p>
